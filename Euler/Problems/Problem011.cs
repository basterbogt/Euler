...
{
    /// <summary>
    /// In the 20×20 grid below, four numbers along a diagonal line have been marked in red. ---> (marked in red.. right.)
    /// 
    ...
    public class Problem011 : Problem
    {
        ...
        public Problem011()
        {
            array = new int[20][]{  new int[] {08,02,22,97,38,15,00,40,00,75,04,05,07,78,52,12,50,77,91,08},
        ...
        public override void Calculate()
        {
            ...
            for (int y = 0; y < height; y++)
            {
                ...
                {
                    result = Math.Max(result, CalculateHorizontal(ref array, x, y));
                    result = Math.Max(result, CalculateVertical(ref array, x, y));
                    result = Math.Max(result, CalculateDiagonalNorthEast(ref array, x, y));
                    result = Math.Max(result, CalculateDiagonalSouthEast(ref array, x, y));
                }
            }
            Print(result);
        }

        private int CalculateHorizontal(ref int[][] array, int x, int y)
        {
            return Number(ref array, x, y) * Number(ref array, x + 1, y) * Number(ref array, x + 2, y) * Number(ref array, x + 3, y);
        }

        private int CalculateVertical(ref int[][] array, int x, int y)
        {
            return Number(ref array, x, y) * Number(ref array, x, y + 1) * Number(ref array, x, y + 2) * Number(ref array, x, y + 3);
        }

        private int CalculateDiagonalNorthEast(ref int[][] array, int x, int y)
        {
            return Number(ref array, x, y) * Number(ref array, x + 1, y + 1) * Number(ref array, x + 2, y + 2) * Number(ref array, x + 3, y + 3);
        }

        private int CalculateDiagonalSouthEast(ref int[][] array, int x, int y)
        {
            return Number(ref array, x, y) * Number(ref array, x + 1, y - 1) * Number(ref array, x + 2, y - 2) * Number(ref array, x + 3, y - 3);
        }
        
        private bool InArray(ref int[][] array, int x, int y)
        {
            if (x < 0 || x >= array[0].Length || y < 0 || y >= array.Length) return false;
            return true;
        }

        private int Number(ref int[][] array, int x, int y)
        {
            if (!InArray(ref array, x, y)) return 0; //Cheesy workaround so i won't have to deal with out of bounce...
            return array[x][y];
        }
